...
for sas in ['1e-24','0.1','1','1e24']:
 ...
 for sbs in ['1e-24','0.3','1','1e24']:
  ...
  ga=dot(a,mv(S,a)); gb=dot(b,mv(S,b)); shape=[contract(M,S) for M in (Ma,Mb,Mc)]
  certify_one(ga/sa,('gain a',sas,sbs)); certify_one(gb/sb,('gain b',sas,sbs))
  if not all(x.contains(0) for x in shape): raise AssertionError(('shape lock',sas,sbs,shape))
  ...
  rows.append({'s_a':sas,'s_b':sbs,'S_Frobenius_squared':str(S2),'closed_norm_squared':str(closed2),'det_S_lock':str(d),'closed_negative_det':str(closedd),'bridge_rate':str(bridge),'shape_rates':[str(x) for x in shape]})
