...
projection_residual=C.norm2v(res)
# y5 is a midpoint search observer.  Project it onto the physically certified D2-fixed subspace before the 3D solve;
# do not require floating midpoint coordinates to satisfy an exact group identity.
...
Binv=[[Bcols[j][i] for j in range(3)] for i in range(3)];rad_s='1e-20' if BITS==160 else ('1e-30' if BITS==256 else '1e-50');R=arb('0 +/- '+rad_s);X=[arb(v.mid())+R for v in a];D=[arb('0 +/- '+rad_s) for _ in range(3)];JX=JG(X);Bg=H.matvec(Binv,g0);center=[a[i]-Bg[i] for i in range(3)];M=H.matmul(Binv,JX);I=H.eye(3);E=[[I[i][j]-M[i][j] for j in range(3)] for i in range(3)];K=[center[i]+v for i,v in enumerate(H.matvec(E,D))];inc=[bool(K[i].lower()>X[i].lower() and K[i].upper()<X[i].upper()) for i in range(3)]
if not all(inc):raise AssertionError(('3D D2 Krawczyk',inc,K,X))
print(json.dumps({'arb_precision_bits':BITS,'status':'PASS','full_T4_feedback_dimension':9,'D2_fixed_T4_feedback_dimension':3,'five_dim_midpoint_projection_residual_mean_square_autopsy':str(projection_residual),'D2_feedback_coordinates':[str(v) for v in a],'Krawczyk_box_radius':rad_s,'Krawczyk_coordinate_inclusions':inc,'Krawczyk_certified_fixed_point':True,'Krawczyk_image':[str(v) for v in K],'interpretation':'The complete nine-dimensional degree-six T4 feedback sector is group-averaged under the proper D2 eigenframe stabilizer of the stationary strain.  Its physical fixed subspace has dimension three, exactly the even-l formula (l+2)/2 at l=4.  The previously certified coupled branch lies in this three-dimensional space.  Re-expressing the exact Hodge/Euler feedback map in these three physical eigenframe channels and applying Krawczyk again certifies the same local coupled servo.  Thus the apparent feedback complexity reduces 9 -> 5 -> 3 when the full physical stabilizer is respected.'},indent=2,allow_nan=False))
