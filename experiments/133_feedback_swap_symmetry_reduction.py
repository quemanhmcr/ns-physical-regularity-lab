...
base_errors={}
for name,V,act in [('u1',st['u1'],H.vpolar_swap_xy),('u3',st['u3'],H.vpolar_swap_xy),('omega2',st['omega'],H.vaxial_swap_xy)]:
    e=C.norm2v(C.vadd(act(V),C.vscale(-1,V))); base_errors[name]=str(e)
...
for tag,cols,sgn in [('fixed',sym['S'],1),('anti',sym['Aanti'],-1)]:
    for k,c in enumerate(cols):
        V=C.combine(c,sym['Y']); err=C.vadd(H.vaxial_swap_xy(V),C.vscale(-sgn,V)); q=C.norm2v(err)
...
asym=[arb(k+1) for k in range(5)]; ysym=H.y_from_sym(asym,sym); fsym=H.feedback_map(st,ysym)['F']; Ffield=C.combine(fsym,sym['Y']); fixederr=C.norm2v(C.vadd(H.vaxial_swap_xy(Ffield),C.vscale(-1,Ffield)))
...
out={
 ...
 'interpretation':'The stationary capacity velocity is polar-invariant under exchange x<->y while its vorticity is invariant under the corresponding axial action det(R)R, and the intrinsic nine-dimensional degree-six toroidal-l4 feedback sector decomposes into a five-dimensional swap-fixed subspace and a four-dimensional anti-fixed subspace.  The Hodge/Euler feedback map is equivariant under the same physical symmetry, so a symmetric coupled branch is governed by only five feedback amplitudes.  This is a symmetry reduction selected by the base flow, not an imposed modal truncation.'}
